...
const (
	...
	ResourceMonitor_StreamInvoke_FullMethodName                 = "/pulumirpc.ResourceMonitor/StreamInvoke"
...
type ResourceMonitorClient interface {
	...
	StreamInvoke(ctx context.Context, in *ResourceInvokeRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[InvokeResponse], error)
...
func (c *resourceMonitorClient) StreamInvoke(ctx context.Context, in *ResourceInvokeRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[InvokeResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ResourceMonitor_ServiceDesc.Streams[0], ResourceMonitor_StreamInvoke_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ResourceInvokeRequest, InvokeResponse]{ClientStream: stream}
...
type ResourceMonitor_StreamInvokeClient = grpc.ServerStreamingClient[InvokeResponse]
...
type ResourceMonitorServer interface {
	...
	StreamInvoke(*ResourceInvokeRequest, grpc.ServerStreamingServer[InvokeResponse]) error
...
func (UnimplementedResourceMonitorServer) StreamInvoke(*ResourceInvokeRequest, grpc.ServerStreamingServer[InvokeResponse]) error {
...
func _ResourceMonitor_StreamInvoke_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ResourceMonitorServer).StreamInvoke(m, &grpc.GenericServerStream[ResourceInvokeRequest, InvokeResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ResourceMonitor_StreamInvokeServer = grpc.ServerStreamingServer[InvokeResponse]
